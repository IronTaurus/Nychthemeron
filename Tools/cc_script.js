...
function generateCard() {
    ...
    if (element.checked == true) {
      if (element.name == "csRangeType") {
        ...
        if (element.value == "None") {
          ...
          document.getElementById("rangeArt_sun").src =
            imgPath + "range" + element.value + ".png";
        }
      }
      if (element.name == "cmRangeType") {
        if (element.value == "None") {
          ...
          document.getElementById("rangeArt_moon").src =
            imgPath + "range" + element.value + ".png";
        }
      }
    }
  ...
  const mapObj = {
    "{ac}":
      "<img src='" +
      imgPath +
      "Symbol_AC.png' height = '22px' width = '16px' margin= '0%'/>",
    "{w}":
      "<img src='" + imgPath + "BaseDmg.png' height = '22' width = '17' />",
    "{sp}":
      "<img src='" +
      imgPath +
      "SpiritCost.png' height = '17' width = '17' style='margin-top: 8px' />",
    "{and}":
      "<img src='" +
      imgPath +
      "and.png' height='16' width='340' style='margin-top: 8px'/> <br>",
    "{or}":
      "<img src='" +
      imgPath +
      "or.png' height='16' width='340' style='margin-top: 8px'/> <br>",
    "{-}": "<img src='" + imgPath + "break.png' height='16' width='340'/> <br>",
    "{1}":
      "<img src='" +
      imgPath +
      "break_first.png' height='16' width='340'/> <br>",
    "{2}":
      "<img src='" +
      imgPath +
      "break_second.png' height='16' width='340'/> <br>",
    "{3}":
      "<img src='" +
      imgPath +
      "break_third.png' height='16' width='340'/> <br>",
    "{melody}":
      "<img src='" +
      imgPath +
      "LingeringMelody.png' height='16' width='340'/> <br>",
  };
  let sunRuleReplaced = sunRuleText.replace(
    /{ac}|{w}|{sp}|{and}|{or}|{-}|{one}|{two}|{three}|{melody}/gi,
    (matched) => mapObj[matched]
  );
  let moonRuleReplaced = moonRuleText.replace(
    /{ac}|{w}|{sp}|{and}|{or}|{-}|{one}|{two}|{three}|{melody}/gi,
    (matched) => mapObj[matched]
  );
  ...
  if (title.innerHTML.length > charLimit) {
    ...
    document.getElementById("CardGen_Title_sun").style.fontSize =
      textSize - 4 + "px";
    textSize = textSize - 4;
  ...
  document.getElementById("flavourText_sun").innerHTML =
    document.getElementById("csFlavor").value;
  var selectedSubclass =
    classDropdown.options[classDropdown.selectedIndex].value;
  ...
  document.getElementById("CardGen_SubClass_sun").innerHTML =
    selectedSubclass + " " + selectedTier;
  ...
  document.getElementById("flavourText_moon").innerHTML =
    document.getElementById("cmFlavor").value;
  var selectedSubclass =
    classDropdown.options[classDropdown.selectedIndex].value;
  var selectedTier = tierDropdown.options[tierDropdown.selectedIndex].value;
  document.getElementById("CardGen_Class_moon").innerHTML = baseClass;
  document.getElementById("CardGen_SubClass_moon").innerHTML =
    selectedSubclass + " " + selectedTier;
}
